{
  ...
  evalDependency = {optional ? false}: dep:
    dep {
      package = {
        name,
        package,
        ...
        want = evalCondition enabled;
      in
        if want && package != null
        then
          (
            if evalConstraint constraint package.version
            then [package]
            else null
          )
        else if optional || !want
        then []
        else null;
    };
  ...
  showDependency = dep:
    dep {
      ...
      in
        if want && package != null
        then "${showConstraint package constraint}"
        else if !want
        then "${name} disabled"
        else "${name} is unknown";
    };

  eval = {name, ...} @ config: depFormula: let
  ...
  evalNative = guessedNativeDepends: nativeDepends: let
    nativeDeps =
      lib.concatMap
      ({nativePackages, ...}: nativePackages)
      (lib.filter ({filter, ...}: filterLib.eval filter) nativeDepends);
  in
    if lib.length nativeDeps > 0
    then nativeDeps
    else lib.filter (x: x != null) guessedNativeDepends;
in {
